...
import { createNewNode } from './js/createnode.js';
...
const pressKey = (event, btn, code) => {
  ...
  if (btn.dataset.type === 'func') {
    ...
    if (code === 'Backspace') {
      let str = inputField.value.substring(0, caret - 1) + inputField.value.substring(caret, inputField.value.length);
      inputField.value = str;
